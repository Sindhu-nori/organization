	...
	private void saveSignatureTemplates(RegisterOrganizationDTO registerOrganizationDTO,
		...
		if (registerOrganizationDTO.getTemplateId() == null || registerOrganizationDTO.getTemplateId().isEmpty()) {
			...
			throw new OrgnizationServiceException(
					messageSource.getMessage(Constant.API_RESPONSE_TEMPLATE_CANT_EMPTY, null, Locale.ENGLISH));
		}
...
//			contactsEmail.setUgpassUserLinkApproved(orgUser.isUgpassUserLinkApproved());
	...
	private List<OrgUser> mapToOrgUserList(List<OrgContactsEmail> orgContactsEmailList) {
		return orgContactsEmailList.stream().map(userEmail -> {
	...
	private List<Integer> mapToTemplateIdList(List<OrganizationSignatureTemplates> list) {
		return list.stream().map(OrganizationSignatureTemplates::getTemplateId).collect(Collectors.toList());
	}

	private List<String> mapToDirectorsEmailList(List<OrganizationDirectors> directorsList) {
		return directorsList.stream().map(OrganizationDirectors::getDirectorsEmailList).collect(Collectors.toList());
	}

	private List<String> mapToDocumentCheckList(List<OrganizationDocumentsCheckBox> documentsCheckBoxList) {
		return documentsCheckBoxList.stream().map(OrganizationDocumentsCheckBox::getDocumentListCheckbox)
				.collect(Collectors.toList());
	}
	...
	private void updateEmailDomain(RegisterOrganizationDTO updateOrganizationDTO,
		...
		if (emailDomain == null) {
			throw new OrgnizationServiceException(
					messageSource.getMessage(Constant.API_ERROR_ORGANIZATION_DETAILS_NOT_FOUND_IN_DOMAIN, null, Locale.ENGLISH));
		}
	...
	private void updateOrgUsers(RegisterOrganizationDTO updateOrganizationDTO, OrganizationDetails organizationDetails)
		...
		for (OrgUser orgUser : updateOrganizationDTO.getOrgUserList()) {
			...
			// Handle notifications for updated users


			// Build and add OrgContactsEmail object
			OrgContactsEmail contactsEmail = buildOrgContactsEmail(orgUser, organizationDetails.getOrganizationUid());
			contactsEmails.add(contactsEmail);
		}
		// Delete existing user records for the organization
//		orgContactsEmailRepository.deleteOrganizationEmailById(organizationDetails.getOrganizationUid());
//
...
//		orgContactsEmailRepository.saveAll(contactsEmails);
		...
		List<OrgContactsEmail> contactsEmails1 = orgContactsEmailRepository.findByOrganizationUid(updateOrganizationDTO.getOrganizationUid());
		System.out.println("Size of deleted business users" + contactsEmails1.size());
		orgContactsEmailRepository.saveAll(contactsEmails);
	}
	...
	@Override
	public ApiResponses addOrganizationEmails(EmailList emailList) {
		try {
			...
			for (String email : emailList.getEmailList()) {
				...
				SubscriberStatus subscriberStatus = subscriberStatusRepository.findBysubscriberUid(subscriberUid);
	...
	public ApiResponses getSubascriberEmailBySearchType(String searchType) {
		try {
			...
			List<String> activeMailList = subscriberList.stream().map(String::trim).filter(mail -> {
				String subscriberUid = subscriberRepository.findByemailId(mail);
				if (subscriberUid == null)
					return false;

				SubscriberStatus subscriberStatus = subscriberStatusRepository.findBysubscriberUid(subscriberUid);
				return subscriberStatus != null && Constant.ACTIVE.equalsIgnoreCase(subscriberStatus.getSubscriberStatus());
			}).collect(Collectors.toList());
	...
	public ApiResponses verifySignedDocumnet(SignatureVerificationContext1 signatureVerificationContext1) {
		try {
			...
			ApiResponses apiResponse = callExternalSignatureVerificationApi(signatureVerificationContext1);
	...
	private String extractCertificateSerialNumber(ApiResponses apiResponse) {
		try {
			...
			LinkedHashMap<String, List<Map<String, String>>> resultMap = (LinkedHashMap<String, List<Map<String, String>>>) apiResponse
					.getResult();
			...
			for (Map.Entry<String, List<Map<String, String>>> entry : resultMap.entrySet()) {
				List<Map<String, String>> signatures = entry.getValue();
				if (signatures.size() > 1)
					return null;

				SignatureDeatils signatureDetails = objectMapper.convertValue(signatures.get(0),
						SignatureDeatils.class);
				return signatureDetails.getCertificateSerialNumber();
			}
			return null;

		} catch (Exception e) {
			logger.error("{} - {}: Failed to extract certificate serial number: {}", CLASS, Utility.getMethodName(),
					e.getMessage(), e);
			return null;
		}
	}
	...
	@Override
	public ApiResponses getUserTemplateDetails(GetTemplateDto getTemplateDto) {
		String methodName = Utility.getMethodName();
		logger.info("{} - {}: Fetching user template details for {}", CLASS, methodName, getTemplateDto);

		if (Objects.isNull(getTemplateDto)) {
			return exceptionHandlerUtil.createErrorResponse(Constant.API_ERROR_GET_TEMPLATES_DTO_REQURIED);
		}
		
		SignatureTemplateDto signatureTemplateDto = new SignatureTemplateDto();
		try {
			OrgContactsEmail orgUser = orgContactsEmailRepository
					.getOrganizationSigntoryDetails(getTemplateDto.getEmail(), getTemplateDto.getOrgId());
			if (orgUser == null) {
				logger.warn("{} - {}: Email not found in UGPass for orgId: {}", CLASS, methodName,
						getTemplateDto.getOrgId());
			...
			if (orgUser.getSubscriberUid() == null || orgUser.getSubscriberUid().isEmpty()) {
				logger.warn("{} - {}: Email not linked to any subscriber for orgId: {}", CLASS, methodName,
						getTemplateDto.getOrgId());
				return AppUtil.createApiResponse(false,
						messageSource.getMessage(Constant.API_ERROR_EMAIL_NOT_LINKED, null, Locale.ENGLISH),
						signatureTemplateDto);
			}

			SubscriberCompleteDetail completeDetail = subscriberCompleteDetailRepoIface
					.getSubscriberSelfieThumbnailBySuid(orgUser.getSubscriberUid());
			if (completeDetail == null || completeDetail.getSelfieThumbnailUri() == null
					|| completeDetail.getSelfieThumbnailUri().isEmpty()) {
				logger.warn("{} - {}: Subscriber selfie thumbnail not found for orgId: {}", CLASS, methodName,
						getTemplateDto.getOrgId());
				return exceptionHandlerUtil.createErrorResponse(Constant.API_ERROR_SUBSCRIBER_SELFIE_NOT_FOUND);
			}

			signatureTemplateDto.setSelfieThumbnail(completeDetail.getSelfieThumbnailUri());
			signatureTemplateDto.setFullName(completeDetail.getFullName());

			// Handle templateId cases
			switch (getTemplateDto.getTemplateId()) {
				case 0:
					return exceptionHandlerUtil.createSuccessResponse(Constant.API_RESPONSE_TEMPLATE_ID, signatureTemplateDto);
				case 1:
					signatureTemplateDto.setDesignation(orgUser.getDesignation());
					return exceptionHandlerUtil.createSuccessResponse(Constant.API_RESPONSE_THIS_EMAIL_HAS_STANDARD_SIGN,
							signatureTemplateDto);
				case 2:
					if (orgUser.getDesignation() != null && !orgUser.getDesignation().isEmpty()) {
						signatureTemplateDto.setDesignation(orgUser.getDesignation());
						return exceptionHandlerUtil.createSuccessResponse(Constant.API_RESPONSE_THIS_EMAIL_HAS_DESIGNATION,
								signatureTemplateDto);
					}
					return exceptionHandlerUtil.createErrorResponse(Constant.API_ERROR_THIS_EMAIL_NO_DESIGNATION);
				case 3:
					if (orgUser.getSignaturePhoto() != null && !orgUser.getSignaturePhoto().isEmpty()) {
						signatureTemplateDto.setDesignation(orgUser.getDesignation());
						return exceptionHandlerUtil.createSuccessResponse(Constant.API_RESPONSE_EMAIL_HAVE_HANDWRITTEN_SIGNATURE,
								signatureTemplateDto);
					}
					return exceptionHandlerUtil.createErrorResponse(Constant.API_ERROR_EMAIL_HAS_NO_HANDWRITTEN_SIGNATURE);
				case 4:
					if (orgUser.getSignaturePhoto() != null && !orgUser.getSignaturePhoto().isEmpty()
							&& orgUser.getDesignation() != null && !orgUser.getDesignation().isEmpty()) {
						signatureTemplateDto.setDesignation(orgUser.getDesignation());
						return exceptionHandlerUtil.createSuccessResponse(
								Constant.API_RESPONSE_EMAIL_HAVE_HANDWRITTEN_SIGNATURE_AND_DESIGNATION, signatureTemplateDto);
					}
					return exceptionHandlerUtil.createErrorResponse(Constant.API_ERROR_EMAIL_HAS_NO_HANDWRITTEN_ANDDESIGNATION);
				default:
					logger.warn("{} - {}: Invalid templateId: {} for orgId: {}", CLASS, methodName,
							getTemplateDto.getTemplateId(), getTemplateDto.getOrgId());
	...
	public ApiResponses sendEmailToSubscriber(EmailReqDto emailReqDto) {
		...
		try {
			...
			switch (res.getStatusCodeValue()) {
				...
				default:
					logger.error("{} - {}: Unexpected response: {}", CLASS, methodName, res.getBody().getMessage());
	...
	public void sendNotification(String fullName, String fcm, boolean link) {
		...
		try {
			...
			if (res.getStatusCodeValue() == 200) {
				...
				logger.error("{} - {}: Notification failed with status: {}", CLASS, methodName,
	...
	public ApiResponses sendEmail(String spocEmail, String orgName) {
		...
		try {
			...
			String errorMsg = res.getStatusCodeValue() == 400 ? "Bad Request" : "Internal Server Error";
	...
	@Override
	public ApiResponses checkTemplates(List<CheckTemplateDto> checkTemplateDtoList) {
		try {
			if (checkTemplateDtoList == null || checkTemplateDtoList.isEmpty()) {
				return exceptionHandlerUtil.createErrorResponse("Template list cannot be null or empty");
			}

			for (CheckTemplateDto checkTemplateDto : checkTemplateDtoList) {
				if (checkTemplateDto.getOrganizationId() == null || checkTemplateDto.getOrganizationId().isEmpty()) {
					return exceptionHandlerUtil.createErrorResponse(Constant.API_ERROR_ORGUID_CANT_BE_NULL_OR_EMPTY);
				}
				if (checkTemplateDto.getSignatureTemplateId() == 0) {
					return exceptionHandlerUtil.createErrorResponse(Constant.CHECK_TEMPLATE_ID_CANNOT_ZERO);
				}
				if (checkTemplateDto.getEsealTemplateId() == 0) {
					return exceptionHandlerUtil.createErrorResponse(Constant.CHECK_TEMPLATE_ID_CANNOT_ZERO);
				}

				OrgContactsEmail orgUser = orgContactsEmailRepository.getOrganizationSigntoryDetailsBySuidAndOuid(checkTemplateDto.getSuid(),checkTemplateDto.getOrganizationId());
				if(orgUser==null){
					return AppUtil.createApiResponse(false, "Business User Not Found", checkTemplateDtoList);

				}

				boolean hasSignature = false;
				boolean hasEseal = true;
				if(checkTemplateDto.getSignatureTemplateId()==1){
					hasSignature=true;
				}
				if(checkTemplateDto.getSignatureTemplateId()==2){
					//check designation
					if(orgUser.getDesignation()==null || orgUser.getDesignation().isEmpty()){
						hasSignature=false;
					}
				...
				else if(checkTemplateDto.getSignatureTemplateId()==3){
					//check handwrittenSign
					if(orgUser.getSignaturePhoto()==null || orgUser.getSignaturePhoto().isEmpty()){
						hasSignature=false;
					}
					else{
				...
				else if(checkTemplateDto.getSignatureTemplateId()==4){
					//check Both
					if(orgUser.getSignaturePhoto()==null || orgUser.getSignaturePhoto().isEmpty() || orgUser.getDesignation()==null || orgUser.getDesignation().isEmpty()){
						hasSignature=false;
					}
					else{
						hasSignature=true;
				...
				checkTemplateDto.setHasSignatureTemplate(hasSignature);
				checkTemplateDto.setHasEsealTemplate(hasEseal);
			}
